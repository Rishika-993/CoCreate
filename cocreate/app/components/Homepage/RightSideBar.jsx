...
const RightSidebar = () => {
  return (
    <aside className="hidden lg:block w-80 fixed right-0 top-8 h-screen overflow-y-auto p-6 pb-24 bg-white dark:bg-black  border-l border-[var(--border)]">
        ...
        <section>
          <h2 className="text-xl font-semibold mb-4 dark:text-white">Trending</h2>
            ...
            {sidebarData.curatedPicks.map((pick, index) => (
              <article key={index} className="flex flex-col space-y-2">
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <span className="text-xs dark:text-gray-300">{pick.author.name}</span>
                  <span className="text-xs text-gray-500 dark:text-gray-400">•</span>
                  <span className="text-xs text-gray-500 dark:text-gray-400">{pick.category}</span>
                </div>
      ...
      <div className="flex justify-between items-center">
        ...
        {/* Categories Section */}
        <section>
          <h2 className="text-xl font-semibold my-4 dark:text-white">Categories</h2>
